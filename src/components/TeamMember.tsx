...
const TeamMember = ({ name, title, description, whatsapp, image, reverse = false }: TeamMemberProps) => {
  return (
    <div className={`flex flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-8 md:gap-16`}>
      ...
      <div className={`text-center ${reverse ? 'md:text-right' : 'md:text-left'} flex-1`}>
        ...
        <p className="font-body text-sm uppercase tracking-wider text-primary mb-4">
          {title}
        </p>
        ...
        <a
          ...
          className="inline-flex items-center gap-2 font-body text-sm font-medium text-primary hover:text-secondary transition-colors duration-300"
        >
